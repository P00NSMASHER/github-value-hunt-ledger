...
- Implementation checkpoint (2026-09-19): **Freight Recovery v10 core v1.2 now passes 66/66 regression tests.** v1.1 added messy customer GPS/TMS physical-event normalization into the permissioned Opstrax event/detention path. v1.2 adds a detention-policy parity firewall: automated detention dollars are allowed only when the controlling customer policy is exactly representable by the current Opstrax adapter. Policies requiring round-up/nearest/exact rounding, arrival-only or appointment-only clocks, different pickup/delivery free time, minimum billable time, tiered rates, required-notice suppression, layover conversion or per-day/per-shipment caps route to **REVIEW_ZERO_ASSERTION** instead of silently inheriting Opstrax defaults. `emoss08/Trenova@95fcf816562025ad9af864ded4a5fce8a555bd65` is used only as a current FSL-restricted challenge oracle for policy dimensions, not embedded/reused in the competing runtime. The validated physical-truth regression remains: 180 minutes observed dwell - 120 free = 60 billable minutes at $75/hour = $75 expected detention; $150 billed => $75 validated variance with evidence SHA. Current artifact: `freight-v10-core-v1.2-policy-parity.zip`.
