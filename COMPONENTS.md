...
## Integrator component delta — 2026-09-20 12:07 ET
- **`Modern-Treasury/modern-treasury-python@406f354a06a98060df0c9f4c242fe56cc65e1526` — 26/30 (A4 B5 C5 D4 E5 F3):** strongest current production-shaped bank-reconciliation bridge. The MIT SDK exposes Payment Orders, ACH/original-trace references, bank-derived Transactions/Transaction Line Items and later Returns/Reversals. First-party service documentation confirms that completed ACH can later return and that bank Transactions originate from underlying bank connectivity. Limitations: hosted API/bank connections and data are external; server-side auto-reconciliation is not public; trace/reference alone is not unique settlement identity; source completeness/freshness is connection-specific. Target: CAP-018 / EXP-003. Next test: exact provider/reference->bank transaction mapping under duplicate/ambiguous/stale/unavailable conditions.
- **`szapata85/ACHInterbank@395a359230eff35e49cc196b49995ed4c49509b9` — 25/30 (A3 B4 C5 D5 E5 F3):** rare fail-closed ACH-return correlation oracle. It attempts exact original-trace, exact trace and then composite business identity; one candidate becomes final, multiple candidates become Ambiguous, zero becomes NotFound, and ambiguous/no-find paths preserve money state while storing review evidence. No root license was found at the pinned revision; standing separate repository-code permission applies, while jurisdiction-specific ACH/CENIT materials remain separately governed. Target: adversarial EXP-003 linkage semantics, not production authority. Next test: duplicate-trace/equal-total/subsequence-return corpus and cross-check against bank-derived observations.
- **`OCA/bank-statement-import@01be32ea...` — 23/30 watch:** CAMT/Ponto import surfaces preserve service/creditor/end-to-end references and provider transaction IDs, but last-seen/overlap cursors do not establish universal source completeness. Use only as ingestion/reference context.
- **`sebastienrousseau/bankstatementparser@b3309b78...` — 22/30 negative oracle:** broad parsing is useful for adversarial fixtures, but substring-based `EXACT_REFERENCE`, fuzzy/first-match behavior and invalid-money normalization are unsafe for realized-money authority. Indexed in REJECTED rather than promoted.