...
- Evidence challenger: `AD-SDL/MADSci@6b1ab6a70ce8b15af7aa8968479c90d9138753d0` preserves one action identity across an immediate lost response, re-queries that same action ID, converts repeated unresolved readback to `UNKNOWN`, and fails the workflow. Red-team evidence also shows the generic workflow retry path can re-enqueue the failed step and create a fresh `ActionRequest`/ULID without mandatory original-state reconciliation; therefore exactly-once physical execution is **not** established.
- Inputs: synthetic Clarity QC/pooling/run-prep state, generated sample sheet, lawful InterOp fixtures, stable run identity, invalid transition, missing/mismatched identity, missing metric artifact, run-metric exception, and one fault-injected physical action where the device applies the side effect but the dispatch response is lost. Use a vendor/device `run_id` when available and preserve it across process restart.
- Ambiguity contract: `dispatch -> same-action/device-run readback -> SUCCEEDED / FAILED / UNKNOWN -> recovery gate`. `UNKNOWN` is terminal for normal progression and may not be converted into a fresh physical action by a generic retry surface. Retry is legal only after independent device/state reconciliation establishes that redispatch is safe or the original action is definitively failed/not-applied.
- Success: deterministic workflow/run linkage, independent sample-sheet validation, rejection of bad identities/transitions, explicit unknowns and replayable provenance; response-loss recovery uses the original identity; restart preserves the same decision; unresolved physical effect blocks downstream work and blocks fresh-ID retry/redispatch.
- Failure: wrong run joined, missing evidence turns green, PHI becomes required, command-mutated local state is accepted as physical proof, standards facade is mistaken for safe control, process restart loses original action identity, or generic retry creates a new physical action while the old effect remains unresolved.
- Next action: execute the Clarity/sample-sheet/InterOp handoff plus the MADSci-shaped actuated-but-response-lost/retry/restart negative before broad device discovery. `AD-SDL/ot2_module` and `RoryMB/simlab` may serve as vendor-ID/simulation harnesses but do not themselves prove safe exactly-once behavior.
