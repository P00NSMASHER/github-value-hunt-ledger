...
## Integrator graph delta — 2026-09-20 20:28 ET
- Freight Pilot Amendment bypass -> **CHALLENGES EXP-001 lifecycle authorization**: post-launch scope mutation must bind a fresh authoritative Activation/launch receipt; self-hash and merely different activation strings are integrity hints, not reauthorization.
- `Xero ACCPAYCREDIT Reference + finite Idempotency-Key replay + Credit Note webhook/readback` -> **STRENGTHENS CAP-016 / EXP-002 positive reconciliation** while **CHALLENGING NOT_APPLIED inference** after TTL; zero observation remains UNKNOWN absent bounded completeness.
- `SmallHeroes/Small_Heroes@ed1da86...` -> **STRENGTHENS CAP-016 / EXP-010** with requested-vs-dispatched refund fencing and provider rediscovery by durable business identity; **DEPENDS_ON** real-provider process-kill, lookup-completeness and settlement evidence for stronger qualification.
- FireDrill exact-revision CI archaeology -> **STRENGTHENS CAP-010 qualification lineage** and **CHALLENGES inherited-green assumptions**: historical E2E success is not current-revision qualification; `HARNESS_FAILED`, `UNQUALIFIED`, and `RECOVERY_FAILED` are distinct non-green states -> **TESTED_BY EXP-004**.
- SAM `FA524026Q0041` current-action divergence -> **STRENGTHENS CAP-011 temporal authority** and **CHALLENGES history-set-complete=>current assumption** -> **TESTED_BY EXP-006** `ALL_ACTION_UUIDS_PRESENT_BUT_NORMALIZED_LATEST_POINTER_WRONG`.
- `Waleed-Ahmad-dev/SiteMate@5e352deb...` -> **STRENGTHENS counter-event architecture** but **NEGATIVE_CONTROL EXP-005** because generic unsupported negative consumption can reopen capacity; append-only history requires provenance-bound reversal authority and exact baseline-version identity.
- `chromConverter@ddf959... exact-head CI` -> **STRENGTHENS CAP-013** with current Agilent/Shimadzu/Waters automated evidence; Thermo remains **PARTIAL/OPEN** because RAW path is CI-skipped. Rights-clean CC0 Thermo input -> **DEPENDS_ON EXP-007** pinned execution + independent oracle before four-vendor PASS.
- No OUT edge added: these changes sharpen capabilities/experiments only; commercial value/revenue evidence remains unchanged.

<!-- INTEGRATOR-R15-2026-09-20T2100-0400 -->
## Integrator graph delta — 2026-09-20 21:00 ET
- `Sigilweaver/OpenTFRaw@63380dff0d25898f5c6e1184087dc590b0d7b6ab` -> **STRENGTHENS CAP-013** as a decoder-lineage-independent Thermo RAW oracle candidate -> **TESTED_BY EXP-007** on `MSV000094032/raw/Lee_CB_03.raw`; its direct raw-byte path, real-file cross-platform CI and numeric-defect history strengthen independence, while documented unsupported DIA metadata/full-corpus-execution gaps prevent vendor-certification or universal-losslessness claims.
- `chromConverter production Thermo path || OpenTFRaw direct RAW path` -> **COMBINES_WITH CAP-013** only when compared before shared normalization; Entab remains a third challenger for disagreements. Different wrapper names over one decoder do not create independent evidence.
