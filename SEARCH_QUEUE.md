...
- Start each coordinated cycle from `intelligence/HUNT_PLAN.md`. V9/V10 assigns one bounded work item per slot using experiment readiness, strategy evidence, capability gaps, saturation, coverage, adjacency, measurement debt, and learned portfolio capacity. Record allocator generation, portfolio-policy generation, slot role, work kind, source ID, and assignment score in every V10 run. Manual overrides are allowed only when explicit and should state why the allocator was overridden.
- Check `intelligence/COVERAGE_REPORT.md` and `EXPLORATION_GAPS.md` for V8 blind spots. Use coverage gaps only when intersected with a valuable capability/search hypothesis; never retain a repo merely because it fills a language/star/age/source quota. Record `COV:` IDs and repository profiles in V8 telemetry.
