...
- Start each coordinated cycle from `intelligence/HUNT_PLAN.md`. V9 assigns one bounded work item per slot using experiment readiness, strategy evidence, capability gaps, saturation, coverage, adjacency and measurement debt. Record the V9 assignment provenance in every run. Manual overrides are allowed only when explicit and should state why the allocator was overridden.
