...
ACTIVE_AGENTS_SQL = """
select agent_id, role_key, display_name, mode, status
from ai_business_os_prod.agents
where status = %s
order by role_key
"""

PORTFOLIO_INITIATIVES_SQL = """
select id, initiative_key, name, business_id, product_id, owner_agent_id, status
from ai_business_os_prod.portfolio_initiatives
where status = %s
order by initiative_key
"""

OPEN_PORTFOLIO_GAPS_SQL = """
select
    id,
    initiative_id,
    metric_key,
...
order by priority desc, created_at asc, metric_key asc
"""

OPEN_AGENT_GOALS_SQL = """
select
    id,
    agent_id,
    goal_type,
...
class ProductionReadBridge:
    ...
    def planning_inputs(self) -> dict[str, list[dict[str, Any]]]:
        return {
            "agents": self._run(ACTIVE_AGENTS_SQL, ("ACTIVE",)),
            "initiatives": self._run(PORTFOLIO_INITIATIVES_SQL, ("ACTIVE",)),
            "data_gaps": self._run(OPEN_PORTFOLIO_GAPS_SQL),
            "open_goals": self._run(OPEN_AGENT_GOALS_SQL, ("ACTIVE", "PENDING")),
        }
