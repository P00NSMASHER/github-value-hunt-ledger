...
- Matched tasks scored: **30** — 09, 10, 11, 12, 13, 16, 17, 18, 19, 23, 24, 25, 26, 27, 28, 30, 31, 32, 33, 34, 37, 38, 39, 40, 41, 44, 45, 46, 47, 48.
- Control matched mean: **24.57/25**.
- Experiment matched mean: **24.80/25**.
- Mean paired difference (Experiment - Control): **+0.23**.
- Median paired difference: **0.0**.
- Pairwise task win / tie / loss for Experiment: **6 / 20 / 4**.
- False promotions: **0 Control / 0 Experiment** among scored results.
- No-find results: **0 Control / 0 Experiment** among scored results.
- Approximate search effort per validated STRONG result remains **Control ~10-11 reported search/deep-inspection units; Experiment ~11-12**. This is directional because result files mix query counts, candidate triage counts, specialist passes and deep inspections. Experiment often spends extra red-team/history effort to reach the same score, but that extra effort materially improved calibration on Tasks 12, 19, 41, 47 and 48.
- Learning slope remains **mildly positive but score-ceiling limited**. Pair 2 is 25→25→25→25→25; Pair 3 24→25→25→25; Pair 4 23→25→24→25→25→25; Pair 5 25→25→24→25→25; Pair 6 25→25→25→25→25 with unmatched Task 42 also scoring 25; Pair 7 24→25→25→25→25. Median early-to-late within-pair change remains about **+0.5**, while most recent tasks tie at the ceiling. Qualitative learning is stronger than the score slope: 09→10→11, 16→17→19, 25→26→27→28, 30→31→32, 37→38→39→40→41→42, and 44→45→46→47→48.
...
| 04 | CONTROL | 25 | 5/5/5/5/5 | `Shreyas2409/freight-intake@49ee48e383aea202cc5ddf15c500a5b39092d8b7` | No | No | Superior/equivalent extraction target with page-grounded facts, deterministic money arithmetic, review gates and adversarial fixtures; production extraction accuracy remains bounded. |
...
| 16 | CONTROL | 21 | 2/5/5/4/5 | `broad-well/recovair-abm@7b3379cb431591c148a26993097a08487ae6886b` | No | No | Deep partial correctly kept WATCH; joint optimization, legality, uncertainty and replay were incomplete. |
...
| 19 | EXPERIMENT | 25 | 5/5/5/5/5 | `KesavamurthyT/Fair-Dispatch-Transparent-Fair-Route-Allocation@90f8768abe3b49e7bd577cd666096a93ff2da3cd` | No | No | Exact target; route/fairness loop, explanations, appeals/overrides and logs are real; CI/governance seams bounded. |
...
| 26 | EXPERIMENT | 25 | 5/5/5/5/5 | `airsim/rmol@6a51f9b90d361a115e39aa57a3329d7723af717f` | No | No | Exact target; dispatch-to-body trace found empty/commented DP and hard-coded-pass tests without overrejecting the real Monte-Carlo chain. |
...
| 28 | EXPERIMENT | 25 | 5/5/5/5/5 | `PrimeIntellect-ai/prime-agent@e311d6495124cf0bdc629c813fc97a39a9a3054d` | No | No | Exact target with trajectory→typed artifact→versioned persistence→later use/rollback tracing; new executable-skill packaging kept separate. |
...
| 47 | CONTROL | 25 | 5/5/5/5/5 | `aiparallel0/freight-audit@e7869162cf9cb23f6d520a0cd71f87cf973d8c28` | No | No | Superior/equivalent negative control: missing authority becomes default/zero, provisional findings feed value, and overlapping findings double-count dollars; REJECT fully justified. |
...
| 48 | EXPERIMENT | 25 | 5/5/5/5/5 | `stojanpetkovic/sawfleet@02c1c52847e3e3f83ae7dc7dcf72ae5f6bb29fbb` | No | No | Superior/equivalent negative control: real marketplace/sync UI sits over an opaque source plane, health lacks freshness/completeness/degraded contracts, and placeholder contact contamination is explicitly handled; correct REJECT as evidence-grade source truth. |
...
| 50 | CONTROL | 25 | 5/5/5/5/5 | `GSA/GSA-Acquisition-NMCARS@09d5b2d...` | No | No | Exact/accepted negative-control result: repository activity looked recent, but source-level comparison to the current responsible authority showed substantive staleness; recent Git history is not current legal/operational authority. |
...
- Not yet separately promoted: Transition-Order + Temporal-Invariant Verification (Task 18 only); Governance-Loop Boundary Verification (Task 19 only); published-plan replanning invariant quartet (Task 13 only); harness-artifact closed loop (Task 28 only); provenance-adapter intersection (Task 34 only); contract→negative-vector→differential-implementation triangulation (Task 45 only); Value-State Aggregation Audit (Task 47 only); database-boundary bypass check (Task 42 only); opaque-upstream health-contract audit (Task 48 only).

## Experiment-wide conclusion
Too early for a winner claim. Across **30 matched tasks**, Experiment has a small mean advantage (**24.80 vs 24.57**), but the **median paired difference is 0** and task outcomes are **6 wins, 20 ties, 4 losses** for Experiment. Neither condition has produced a scored false promotion or no-find result. The experimental architecture is generating transferable verification/search methods and has improved calibration on several adversarial tasks, but score separation remains small and the experimental path still appears somewhat more expensive. The evidence does **not** justify declaring the Experiment architecture the winner; continue until materially more matched tasks or all 50 tasks complete.