...
LESSON: For evidence-grade entity resolution, fields are not enough. Require a three-layer identity proof: (1) authoritative source-system identity/hierarchy ingestion, (2) deterministic compound keys/reference semantics, and (3) negative tests that reject ambiguous identity or broken references. Task 11 showed that the most commercially valuable behavior was not UEI storage itself but fail-closed DUNS/UEI ambiguity plus tested PIID/ParentAwardId linkage. This is a candidate lesson only; do not promote it to SEARCH_SKILLS until it succeeds on another distinct benchmark task or Hunt 15 approves it.
