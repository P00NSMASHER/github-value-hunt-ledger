...
`ProviderOutcomeReceipt` makes provider identity, processing phase, effect scope, terminality, partial-effect risk and economic-fingerprint agreement explicit. A reviewed `ProviderEndpointContract` must additionally bind that telemetry to one credit-capable interface and prove where its phase sits relative to financial posting. Without that contract, even an exact single-effect `PreProcessingError` remains `UNKNOWN`. With the planted verified contract, the Dynamics recurring-integration adapter admits only `PreProcessingError` as `NOT_APPLIED`; `ProcessedWithErrors`, `PostProcessingFailed`, generic `Failed`, ambiguous `Canceled`, mismatched identity and batch-scoped receipts remain `UNKNOWN`. A `Processed` status becomes `APPLIED` only when the exact economic fingerprint also matches.

The planted Dynamics endpoint contract is fixture input, not a claim about a live tenant. Current public documentation separately establishes generic vendor-invoice header/line import entities and vendor-credit-note business workflows, but does not bind the import entities to a credit-note posting boundary. The adapter therefore remains fail-closed until an authorized sandbox establishes that binding.
