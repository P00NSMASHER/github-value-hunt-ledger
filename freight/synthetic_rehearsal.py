...
def run_rehearsal() -> dict:
    ...
    authority = AuthorityRef("auth-1","cust","carrier","USD","src-auth-1")
    truth = freeze_truth(
        population,
        (authority,),
        (
    ...
    ledger.apply(SettlementEvent("settle-1","f-1",2000,"USD","settle-proof-1"))
    ledger.apply(SettlementEvent("settle-2","f-2",2500,"USD","settle-proof-2"))
    ...
    with tempfile.TemporaryDirectory() as td:
        store = SettlementStore(Path(td) / "settlement.sqlite3")
        store.create_claim(
            RecoveryClaim(
                "f-1","inv-1","carrier","buyer-synthetic","USD",2500,
                "2026-09-20T10:00:00Z","claim-src-f1",False,
            )
        )
        store.create_claim(
            RecoveryClaim(
                "f-2","inv-2","carrier","buyer-synthetic","USD",2500,
                "2026-09-20T10:00:00Z","claim-src-f2",True,
            )
        )
        store.ingest_event(
            SettlementEventRecord(
                "e-1","inv-1","carrier","buyer-synthetic","USD",2000,
                "2026-09-21T10:00:00Z","store-settle-1","CREDIT-MEMO",
            )
        )
        # Partial settlement is intentionally human-reviewed.
        ...
        store.ingest_event(
            SettlementEventRecord(
                "e-2","inv-2","carrier","buyer-synthetic","USD",2500,
