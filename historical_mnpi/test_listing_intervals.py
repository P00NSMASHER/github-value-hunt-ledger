...
class ListingIntervalTests(unittest.TestCase):
    ...
    def test_real_g2_current_batches_expand_2618_of_3828(self):
        ...
        self.assertEqual(len(intervals), 95)
        self.assertEqual(len(result.resolved), 2618)
        self.assertEqual(len(result.unresolved), 1210)
    ...
    def test_materialized_listing_metadata_matches_interval_expansion(self):
        ...
        self.assertEqual(len(actual), 2618)
        expected_rows = {
            (
                item.symbol,
                item.session_date,
