...
class RealCorpusWorkspaceTests(unittest.TestCase):
    ...
    def test_g2_first_seven_batches_resolve_902_listing_observations(self):
        import_dir = CORPUS_DIR / "import"
        files = {
            name: (import_dir / name).read_text(encoding="utf-8")
            for name in (
        ...
        imported = import_real_corpus_metadata(files)
        self.assertEqual(len(imported.listings), 902)
        ...
        self.assertEqual(
            ...
            {"ADI": 44, "BA": 22, "CA": 44, "CAT": 22, "CGNX": 44, "CNMD": 22, "COLM": 44, "CREE": 44, "DE": 22, "DGI": 66, "EHTH": 44, "F": 22, "GILD": 22, "HON": 22, "IDTI": 44, "ILMN": 44, "JNPR": 88, "MDU": 44, "NKE": 22, "PNRA": 88, "SBUX": 22, "VMW": 66},
        )
        self.assertEqual(
    ...
    def test_g2_manifest_is_transparent_about_full_compact_index(self):
        ...
        self.assertEqual(manifest["listing_intervals_verified"], 22)
        self.assertEqual(manifest["listing_batches_completed"], 7)
        self.assertTrue(manifest["listing_full_compact_index_loaded"])
        self.assertEqual(manifest["listing_requirement_compact_rows"], 146)
        self.assertEqual(manifest["listing_metadata_unresolved"], 2926)
