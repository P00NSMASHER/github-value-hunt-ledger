...
class RealCorpusWorkspaceTests(unittest.TestCase):
    ...
    def test_g2_first_eight_batches_resolve_1078_listing_observations(self):
        import_dir = CORPUS_DIR / "import"
        files = {
            name: (import_dir / name).read_text(encoding="utf-8")
            for name in (
        ...
        imported = import_real_corpus_metadata(files)
        self.assertEqual(len(imported.listings), 1078)
    ...
    def test_g2_manifest_is_transparent_about_full_compact_index(self):
        ...
        self.assertEqual(manifest["listing_metadata_unresolved"], 2750)
    ...
    def test_listing_requirement_index_is_frozen_and_complete(self):
    ...
    def test_g2_batch_one_resolves_three_intervals_and_66_rows(self):
        import_dir = CORPUS_DIR / "import"
        ...
        self.assertEqual(
            {row["primary_exchange"] for row in rows if row["symbol"] == "CNMD"},
            {"NASDAQ"},
        )
        self.assertEqual(
            {row["primary_exchange"] for row in rows if row["symbol"] == "CAT"},
            {"NYSE"},
        )
        self.assertEqual(
            {row["primary_exchange"] for row in rows if row["symbol"] == "GILD"},
            {"NASDAQ"},
        )
        self.assertTrue(all(
        ...
        self.assertTrue(all(
            row["data_class"] == "PUBLIC_OR_AUTHORIZED_HISTORICAL"
            for row in rows
        ))
        ...
        self.assertEqual(manifest["listing_requirement_symbols"], 146)
        self.assertEqual(manifest["listing_requirement_observations"], 3828)
