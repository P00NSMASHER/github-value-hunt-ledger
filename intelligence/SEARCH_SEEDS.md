...
- Next action: run official worked examples plus identical UBL/CII fixtures across independent validators and route disagreement or missing provenance to source review.
- Query recipe: capability:cap-008
- Strategy: STRAT:rule-period-authority-version-audit
- Objective: OBJ:current-rule-authority
- Capability/experiment: CAP-008, EXP-002
- Saturation/status: INSUFFICIENT / adjustment +0 / action MEASURE_MORE
- Why now: CAP-008 is a current high-information gap (gap score 4, prior run attention 3). Saturation: INSUFFICIENT (+0 priority). Missing piece: pin official rule-pack/table bytes, source identity, effective interval, parser/map digest, imported-row-set digest and validator/runtime revision in one authority receipt.
