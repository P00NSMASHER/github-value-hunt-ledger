...
Read intelligence/HUNTER_MISSION.md, then intelligence/NETWORK_PRIORS.md as a compact navigation layer, then the relevant current HUNT_PLAN packet. NETWORK_PRIORS is never authority: current assignment packets, domain STOP gates, exact evidence, leases and verifier results outrank it. Preserve the original lane as a fallback specialty, not a reason to duplicate another worker. Read current capability/experiment/STOP status before execution. Then read only the entries in intelligence/COORDINATION_BOARD.md that match the current assignment, capability, experiment or candidate fingerprint; the board is cross-hunter context, not authority. Generated scores are scheduling priorities.
...
3. Reconcile coordination packets into intelligence/COORDINATION_BOARD.md before broad new searching: close duplicates, preserve unresolved proof obligations, and route capability deltas/negative knowledge/experiment changes to their existing authoritative ledgers. Then reconcile completed experiments to outcomes with originating search IDs. Review intelligence/REPAIR_QUEUE.md before inventing new learning machinery: `NEEDS_REPRODUCTION` means reproduce and submit a real failure packet first; `READY_FOR_REPAIR` authorizes only one bounded candidate repair inside the named logical target, never a live/global edit. Prioritize actual result/verification debt over new policy machinery.
4. Update central capability/experiment/MASTER/queue/skills only when evidence changed; keep promotions small and reversible. For hunter-system repair, review REPAIR_CANDIDATE_INTAKE.json and SKILL_EVAL_QUEUE.md: a candidate must be bound to the current repair-task hash and may advance only to READY_FOR_SKILL_EVAL after all stated regression tests pass. Then review SKILL_EVAL_RESULT_INTAKE.json / SKILL_PROMOTION_QUEUE.md: only independently evaluated, split-isolated candidates with zero hard regressions and sufficient held-out delta may become STAGED_MUTATION. STAGED_MUTATION is not live/canary/global. No candidate promotions based solely on scores.
