...
5. Freeze baseline and candidate artifact references **and their SHA-256 content digests**, plus the complete diff SHA-256. Baseline and candidate hashes must differ.
6. Copy the repair task's exact regression-test requirement into the candidate packet; every stated test must pass and the complete regression-evidence bundle must have a SHA-256 digest.
7. Freeze the decision-history artifact with its own SHA-256 digest so the repair rationale cannot change after intake.
8. Do not include sensitive/confidential payloads or benchmark-contaminated evidence.
9. Spool files are immutable. Correct a bad packet with a new candidate ID rather than rewriting history.
