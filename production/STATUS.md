...
- Skill-evaluation result intake now binds independent evaluator evidence to the exact candidate/task hashes, enforces mutate-dev vs promotion-test isolation and independent mutator/evaluator identities, and emits only STAGED_MUTATION plus a gated promotion-evidence queue; automatic live/global use remains disabled.
